...
[Documentation](docs/README.md) · [Vision](VISION.md) · [Current status](docs/roadmap/current-status.md) · [Sprint 6 completion](docs/roadmap/sprint-6-completion-record.md) · [Aster contracts](docs/product/aster-contract-baseline.md) · [Operational simplicity](docs/decisions/0011-operational-simplicity-and-durable-workflows.md) · [Consumer-first boundary](docs/decisions/0010-consumer-first-provider-independent-boundary.md) · [Funding baseline](docs/economics/README.md) · [Legacy stewardship proposal](docs/decisions/0009-health-data-legacy-and-post-mortem-stewardship.md) · [Roadmap](ROADMAP.md) · [Governance](GOVERNANCE.md) · [Contributing](CONTRIBUTING.md)
...
- **See what exists now:** [Current Project Status](docs/roadmap/current-status.md) → [Sprint 6 Completion Record](docs/roadmap/sprint-6-completion-record.md) → [Sprint Roadmap](docs/roadmap/sprints.md)
- **Review the Aster boundary:** [Aster Contract Baseline](docs/product/aster-contract-baseline.md) → [Cross-Contract Reconciliation](docs/architecture/aster-sprint-6-cross-contract-reconciliation.md) → [Control and Evidence Map](docs/architecture/aster-sprint-6-control-and-evidence-map.md) → [Holdpoints and Unresolved Work](docs/architecture/aster-sprint-6-specialist-holdpoint-and-unresolved-work-register.md)
- **Apply the operational-simplicity baseline:** [Decision 0011](docs/decisions/0011-operational-simplicity-and-durable-workflows.md) → [Operational Architecture](docs/architecture/operational-simplicity-and-durable-workflows.md) → [Mission Traceability](docs/architecture/mission-to-runtime-traceability.md) → [Cross-Phase Workstream](docs/roadmap/operational-simplicity-workstream.md)
- **Apply the consumer-first boundary:** [Decision 0010](docs/decisions/0010-consumer-first-provider-independent-boundary.md) → [Architecture Rationale](docs/architecture/consumer-first-provider-independent-boundary.md) → [Repository Reconciliation](docs/roadmap/consumer-first-provider-independent-reconciliation.md) → [Cross-Phase Workstream](docs/roadmap/consumer-first-provider-independent-workstream.md)
...
Sprints 0–5 and the Phase 0 Funding and Sponsorship Baseline are complete and merged. Decisions 0010 and 0011 are accepted Phase 0 baselines. Decision 0009 remains a proposed future institutional boundary.

**Sprint 6 — Aster Contracts and AI Governance is complete on draft PR #48 for its accepted public contract, deterministic validation, public synthetic fixture, provider-governance, compatibility, migration, control-mapping, holdpoint, and documentation scope. Explicit founding-steward acceptance and merge remain pending.**

No Sprint 6 artifact activates a production runtime. After explicit acceptance and squash merge, Sprint 7 — Forge MCP and Agent Safety becomes the next numbered design-to-build boundary within its public-documentation and synthetic-data scope.
...
- [Decision 0008](docs/decisions/0008-funding-and-sponsorship-baseline.md) and `docs/economics` merged through PR #38 as squash commit `32ac27bfb35ed64f34c64108a5d54c375d429593`, establishing the funding and sponsorship doctrine, public record contracts, fictional evidence, conflict and concentration controls, public-good underwriting, infrastructure exit requirements, and future operational gates.
...
- Accepted [Decision 0010](docs/decisions/0010-consumer-first-provider-independent-boundary.md) makes the person the provider-independent continuity layer while treating clinics, EHRs, payers, laboratories, devices, exchanges, and other institutions as important sources, destinations, and future partners connected through versioned adapters rather than automatic owners of Chronicle meaning or product authority.
- Accepted [Decision 0011](docs/decisions/0011-operational-simplicity-and-durable-workflows.md) makes the modular monolith, small responsive path, durable deferred work, local simulation, provider-independent adapters, and evidence-gated decomposition explicit implementation constraints rather than informal developer preferences.
- The [Pre-Sprint 6 Alignment Review](docs/roadmap/pre-sprint-6-alignment-review.md) merged through PR #46 as squash commit `4dfd39e7aa02ffe1ef3f5ba296378b29bd078047`, reconciling those boundaries with the Aster handoff.
- The [Sprint 6 Completion Record](docs/roadmap/sprint-6-completion-record.md) maps all deliverables and accepted criteria to public contracts, deterministic validators, public synthetic fixtures, compatibility and migration evidence, open specialist holdpoints, and unresolved production work on draft PR #48.

The merged House of Keys and security work remain design and synthetic-evidence baselines rather than production permission or security systems. Decision 0008 is doctrine rather than authority to accept or operate money. Decisions 0010 and 0011 are accepted constraints but do not establish a connector, clinical use, enterprise product, workflow runtime, measured performance, or validated market evidence. Sprint 6 is likewise a pre-stable contract and public-synthetic evidence baseline rather than production AI. Proposed Decision 0009 does not establish legal sufficiency or a production succession system.
...
- Review and explicitly accept or reject the [Sprint 6 Completion Record](docs/roadmap/sprint-6-completion-record.md), then squash merge PR #48 without weakening its production or specialist gates.
- Begin Sprint 7 only after Sprint 6 acceptance and merge, and keep Forge MCP limited to public documentation and synthetic data.
- Keep production AI providers, private context, provider egress, retrieval, memory storage, identity, persistence, durable execution, tools, connectors, clinical behavior, deployment, and specialist approval behind the [Sprint 6 Holdpoint and Unresolved-Work Register](docs/architecture/aster-sprint-6-specialist-holdpoint-and-unresolved-work-register.md).
...
- Publish the initial key-person dependency, succession, founder-reserved-power, and founder-subsidy or economic-dependency records required by the institutional decisions and roadmap.
- Recover and catalogue the historical HealthDAO, CureDAO, and Calypso’s Promise governance notes.
- Verify administrative branch protections and required checks, decide the commit-level DCO transition, and add distributed code ownership only when qualified second owners can provide real review.
- Measure clean installation, startup, focused validation, package tests, and full validation before accepting numeric operability budgets.
- Obtain named specialist review before sensitive, clinical, privacy, security, accessibility, provider, legal, or operational content is represented as approved.
- Preserve the completed Sprint 5 and Sprint 6 threat, authority, isolation, lifecycle, provider, compatibility, migration, residual-risk, and specialist-holdpoint boundaries before any production data path.
- Keep priority weights, weighted governance, treasury, ownership, token, blockchain, provider placement, connector ranking, affiliate referrals, enterprise economics, service topology, event sourcing, and local-first synchronization unresolved until evidence supports a specific design.
...
- Read [Decision 0011](docs/decisions/0011-operational-simplicity-and-durable-workflows.md) for the boundary between necessary complexity and operational architecture that remains understandable, testable, replaceable, and corrigible.
- Read [Decision 0010](docs/decisions/0010-consumer-first-provider-independent-boundary.md) for the boundary between institutional interoperability and canonical product authority.
...
- Review active [AS-0013](docs/governance/assumption-AS-0013-operational-simplicity-durable-value.md), which treats a modular, batch-oriented, locally simulatable core as a falsifiable implementation hypothesis rather than an identity claim or ban on justified services.
- Review active [AS-0012](docs/governance/assumption-AS-0012-consumer-first-continuity-value.md), which treats consumer-first market and product sequencing as a falsifiable hypothesis rather than an ideology or permanent ban on institutional collaboration.
...
- [`packages/aster`](packages/aster) — pre-stable provider-independent Aster contracts, validators, public synthetic fixtures, compatibility, and migration evidence on draft PR #48; no production model or provider runtime
...
- [`docs/economics`](docs/economics/README.md) — merged funding and sponsorship doctrine, public register contracts, fictional examples, validation, and design tabletops; no operating finance, provider, connector, or enterprise relationship
- [`docs`](docs/README.md) — frozen foundations, architecture, decisions, governance, website briefs, policies, sprint records, and the Sprint 6 completion review
...
- `apps/mcp-forge` — next planned public documentation and synthetic-data agent tooling after Sprint 6 acceptance
- `apps/mcp-chronicle` — planned private, policy-controlled agent tools behind later production gates
- production Aster provider, retrieval, memory, media-processing, identity, persistence, permission, tool, queue, scheduler, workflow, incident, and deletion operations
