...
- **Production website:** live on the canonical domains through Vercel deployment `dpl_3V2e76y1fwrR19j1BzUFpo9U9kjp`
- **Release control:** Git-triggered Vercel deployment is disabled; the retained configuration baseline is `a5146237356f58e8d28343e90918b70a418bccbb`
- **Current repository work:** post-Sprint 8 reconciliation, newsletter gate #63, and pre-Sprint 9 alignment issue #64
- **Sprint 9:** planned and not started
- **Newsletter:** Path A preserve-and-activate selected for narrow Phase 0 updates; implementation is in review and production verification remains open
...
Every route, game mechanic, data flow, model, tool, provider, funding relationship, signup surface, deployment, and governance action remains subordinate to that purpose.
...
The deployment reached `READY`, assigned the apex and `www` domains, served the accepted homepage with HTTP 200, and showed no immediate runtime-error clusters. The framework correction remains in repository configuration while automatic Git deployment was restored to disabled.

The controlling transition record is [Post-Sprint 8 Reconciliation and Sprint 9 Preparation](post-sprint-8-reconciliation-and-sprint-9-preparation.md).
...
- permanent source validation and isolated local production-preview evidence; and
...
The implementation reuses the existing server-only connection:
...
The implementation remains **experimental** until it is merged, deployed, tested end to end against the existing private provider route, reconciled with rollback evidence, and explicitly accepted through issue #63. Real subscriber addresses, webhook URLs, tokens, screenshots, and protected incident evidence must never be posted publicly.
...
- [`apps/site`](../../apps/site) — live public Next.js website with manual release controls and bounded newsletter work under issue #63
...
- final acceptance and deployed verification of newsletter gate #63;
