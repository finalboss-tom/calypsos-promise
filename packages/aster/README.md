...
- `validate.ts`, `validate-role-contracts.ts`, `validate-proposal.ts`, `validate-intent.ts`, `validate-source-recall.ts`, `validate-memory.ts`, `validate-untrusted-input.ts`, `validate-work-lifecycle.ts`, and `validate-provider-governance.ts` deterministically validate the corresponding public contracts.
