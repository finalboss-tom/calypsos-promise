...
const homepage = (
  await Promise.all(
    ["hero", "game", "aster", "ogygia", "promise", "status", "expedition"].map(
      (name) => read(`${name}.html`),
    ),
  )
).join("");
const page = (title, description, content) =>
  shell
    .replace("{{TITLE}}", title)
...
const pages = new Map([
  [
    "/",
    page(
      "Calypso’s Promise — Build Your Living Chronicle",
      "An open-source, narrative-driven health game planned for Ogygia.",
      homepage,
    ),
  ],
  [
    "/privacy",
    page(
      "Signup Privacy — Calypso’s Promise",
      "How Founding Expedition email signups are handled.",
      await read("privacy.html"),
    ),
  ],
  [
    "/joined",
    page(
      "Founding Expedition — Calypso’s Promise",
      "Confirmation for the Founding Expedition.",
      await read("joined.html"),
    ),
  ],
]);
...
export function createSiteServer() {
  return createServer(async (request, response) => {
    ...
    if (url.pathname === "/api/join") {
      ...
      return handleSignup(request, (status, value) =>
    ...
    if (pages.has(pathname))
      return send(
        response,
        200,
        request.method === "HEAD" ? "" : pages.get(pathname),
        {
