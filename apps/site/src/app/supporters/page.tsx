...
export default async function SupportersPage() {
  ...
  if (!promise) throw new Error("The configured Promise version is unavailable.");

  const foundingRemaining = Math.max(0, 1_000 - totals.founding_numbers_assigned);
  ...
  return (
    <article className={styles.page}>
      ...
      <section className={styles.metrics} aria-label="Supporter movement totals">
        <div>
      ...
      <section className={styles.promiseCard} aria-labelledby="promise-text-title">
      ...
      <section className={styles.boundaries} aria-labelledby="boundaries-title">
        <div>
          ...
          <h2 id="boundaries-title">Support does not purchase or surrender anything.</h2>
        </div>
        <ul>
          ...
          <li>Research, newsletter, and public-listing consent remain separate.</li>
          <li>Supporter status does not grant governance or gameplay advantage.</li>
          <li>You may remain private and still be counted.</li>
        </ul>
      </section>

      <section className={styles.publicSupporters} aria-labelledby="public-supporters-title">
        <div className={styles.sectionHeading}>
          <div>
            <p className="eyebrow">Public by separate consent</p>
            <h2 id="public-supporters-title">Verified supporters who chose to be named</h2>
          </div>
          <p>
            Private supporters are included in the total above but never appear
            here. Public profiles contain only fields the supporter chose to publish.
          </p>
        </div>
              ...
              <li key={supporter.profile_slug} className={styles.supporterCard}>
                <div className={styles.supporterIdentity}>
                  ...
                  {typeof supporter.founding_number === "number" ? (
                    <span>Founding Supporter #{String(supporter.founding_number).padStart(4, "0")}</span>
                  ) : (
                    <span>Verified Supporter</span>
                  )}
                </div>
                {supporter.broad_region ? <p>{supporter.broad_region}</p> : null}
      ...
      <section className={styles.enrollment} aria-labelledby="enrollment-title">
        <div className={styles.enrollmentIntro}>
          ...
          <h2 id="enrollment-title">Affirm, verify, and choose your visibility.</h2>
          <p>
