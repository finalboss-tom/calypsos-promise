...
export default function PrivacyPage() {
  return (
    <article className="reading-panel">
      ...
      <p>
        Do not submit health, medical, genetic, wearable, location, Chronicle,
        or other sensitive information. The form has no field for those
        categories.
      </p>
      ...
      <p>
        ...
        {providerDescription}. The webhook address and any token remain
        server-only Vercel environment variables. Subscriber addresses are not
        written to GitHub, public logs, website analytics, or the open-source
        repository.
      </p>

      <h2>Retention, correction, and deletion</h2>
      <p>
        The list is retained only for the Founding Expedition update purpose
        while Phase 0 remains active, unless you unsubscribe or request deletion
        sooner. The storage and provider choice must be reviewed again at Phase
        0 exit or before migration to another newsletter system.
      </p>
      <p>
      ...
      <p>
        The form uses explicit consent, email validation, a bot honeypot,
        bounded request size, best-effort per-source throttling, an HTTPS-only
        server webhook, and a delivery timeout. The in-memory throttle is not a
        durable distributed rate limiter and remains an explicit Phase 0
        limitation. Provider failure returns a public-safe error and does not
        silently claim a successful signup.
      </p>
      <p>
        If the webhook or private sheet is suspected of compromise, intake can
        be disabled by removing the server configuration or restoring the
        no-intake route. Protected incident details and real subscriber records
      ...
      <p>
        ...
        verification, limitations, and founding-steward acceptance are
        reconciled.
      </p>
